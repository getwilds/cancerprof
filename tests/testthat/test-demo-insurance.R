...
test_that("Output data type is correct", {
  skip_on_cran()
  output <- demo_insurance(
    "usa", "state", "% Insured in demographic group, all income levels",
...
for (option in insurance_options) {
  test_that("demo-insurance returns non-empty data frame", {
    skip_on_cran()
    expect_true(is.data.frame(demo_insurance(
      "usa", "state", option, "both sexes",
...
test_that("demo-insurance has correct number of columns", {
  skip_on_cran()
  df <- demo_insurance(
    "wa", "hsa", "% Insured in demographic group, all income levels",
...
test_that("demo-insurance handles invalid insurance parameters", {
  ...
  expect_error(
    demo_insurance(
      "wa", "county", "% Insured in demographic group, all income levels",
