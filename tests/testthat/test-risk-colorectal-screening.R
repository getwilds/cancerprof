...
test_that("Output data type is correct", {
  skip_on_cran()
  output <- risk_colorectal_screening(
...
screening_options <- list(
  ...
  screening3 = risk_colorectal_screening(
    "had colonoscopy in past 10 years, ages 50-75",
    area = "wa"
  ),
  screening4 = risk_colorectal_screening(
    "home blood stool test in the past year, ages 45-75",
    "all races (includes hispanic)", "both sexes"
  ),
  screening5 = risk_colorectal_screening(
...
test_that("risk-colorectal-screening handles invalid parameters", {
  ...
  expect_error(
    ...
    paste(
      "for this screening type,",
      "area must NOT be NULL and Race and Sex must be NULL"
    )
  )
  expect_error(
    risk_colorectal_screening(
